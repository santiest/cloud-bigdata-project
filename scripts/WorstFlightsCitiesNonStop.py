...
spark = (SparkSession.builder.master("local[4]")
            .appName("WorstFlightsCitiesNonStop")
            .getOrCreate()) 

df = spark.read.option("header",True).csv(env.getFileName())

# Average values
result_df = df.filter(df.isNonStop == "True")
result_df = df.withColumn("totalFare", col("totalFare").cast("float")).groupBy("startingAirport","destinationAirport").max("totalFare")

# Write to file
result_df.sort("startingAirport").write.option("header",True).csv("WorstFlightsCitiesNonStop")

# spark-submit WorstFlightsCitiesNonStop.py